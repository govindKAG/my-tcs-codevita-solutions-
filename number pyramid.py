...
def Input(num): #this funtion prints out the answer
    numberList = []
    number = int((num**2 + num)/2) # how many numnbers there are in n lines 
    if num >= 1:
        for eachNum in range(number):
        ...
        try:
            while(True):
                for temp in range(num-a,0,-1): # leading spaces
                    print(" ",end="")
                for b in range(a):
